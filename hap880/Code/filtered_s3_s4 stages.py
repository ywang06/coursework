...
avg_duration = 359.99681511174566
...
df_grouped_diags['diags_all'] = df_grouped_diags.prncpal_group_list ;
...
df_all_diags_s3_history = df_grouped_diags_s3_history.groupby('dsysrtky',as_index=False).agg({'diags_all':'sum', 'min_dt_s3': 'max', 's3_s4_duration': 'max'})
...
df_all_diags_s3_history['start'+s3_code] = df_all_diags_s3_history.dsysrtky.apply(lambda x:diag_identification_date(dict_diag_thru_dt_list_all[str(x), s3_code]))

df_all_diags_s3_history['start'+s4_code] = df_all_diags_s3_history.dsysrtky.apply(lambda x:diag_identification_date(dict_diag_thru_dt_list_all[str(x), s4_code]))

df_all_diags_s3_history['new_s3s4_duration'] = df_all_diags_s3_history.dsysrtky.apply(lambda x:days_between(diag_identification_date(dict_diag_thru_dt_list_all[str(x), s4_code]), diag_identification_date(dict_diag_thru_dt_list_all[str(x), s3_code])))

df_all_diags_s3_history['valid_s3_s4'] =  df_all_diags_s3_history.dsysrtky.apply(lambda x: 1 if (diag_identification_date(dict_diag_thru_dt_list_all[str(x), s4_code])!= today_date and diag_identification_date(dict_diag_thru_dt_list_all[str(x), s3_code])!= today_date) else 0)

old_s3 = lambda x: days_between(x['start5854'],x['min_dt_s3'])
df_all_diags_s3_history['old_s3_duration'] = df_all_diags_s3_history.apply(old_s3,axis=1)

# df_all_diags_s3_history['s3s4_duration'] = df_all_diags_s3_history.apply(lambda x: x['start'+s3_code] - x['start'+s4_code])



df_all_diags_s3_history = df_all_diags_s3_history[df_all_diags_s3_history.valid_s3_s4 == 1]





df_all_diags_s3_history = df_all_diags_s3_history.assign(bi_label=(df_all_diags_s3_history.old_s3_duration < threshold_durationg))
